        ...
        <v-form ref="form" autocomplete="true" :disabled="formDisabled" @submit.prevent="onAction()">
          ...
          <v-card-text>
            ...
            <v-row class="d-flex flex-row mt-2" style="border-top: 2px solid black; border-bottom: 2px solid black">
              <v-col cols="4" class="text-center" style="border-right: 2px solid black">
                <h3>บริษัทประเสริฐผลรุ่งเรืองนครพนม จำกัด</h3>
                <h3>ที่อยู่ : 216/111 นิตโย ตำบลในเมือง อำเภอเมืองนครพนม นครพนม 48000</h3>
            ...
            <!-- กรอบข้อมูลช่อง 13 -->
            <v-row class="d-flex flex-row mb-2" style="border-bottom: 2px solid black">
              <v-col>
                <v-menu
    ...
    async onAction() {
      if (this.$refs.form.validate()) {
        ...
        if (this.formData.action == "add") {
          ...
          const response = await apiFinancial.update(this.formData.id, this.formData);
          // console.log(response);
          this.$refs.form.reset();
