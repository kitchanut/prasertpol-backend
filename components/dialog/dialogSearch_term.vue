        ...
        >
          ...
          <v-card-text>
            <v-container>
              <v-row>
                <v-col cols="12">
                  <v-text-field
                    ...
                    label="ชื่อคำ"
                    append-icon=""
                    v-model="formData.search_name"
                    id="formData.search_name"
              ...
              <v-radio-group
                class="mt-1"
                v-model="formData.search_active"
                id="formData.search_active"
                name="formData.search_active"
                :rules="[(value) => !!value]"
                row
              >
                <template>
  ...
  methods: {
    async onAction(id) {
      if (this.$refs.form.validate()) {
        ...
        if (this.action == "add") {
          const response = await apiSearch_term.store(this.formData);
          ...
          const response = await apiSearch_term.update(id, this.formData);
          this.$refs.form.reset();
    ...
    async dialog() {
      ...
      if (this.$props.dialog) {
        if (this.action == "add") {
          ...
          this.formDataLoading = true;
          const response = await apiSearch_term.show(this.id);
          this.$nextTick(() => {
            this.$refs.form.reset();
            const self = this;
            this.$nextTick(async () => {
