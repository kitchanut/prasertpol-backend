        ...
        >
          ...
          <v-card-text>
            <v-container>
              ...
              <v-radio-group
                v-model="formData.partner_companie_active"
                id="formData.partner_companie_active"
                name="formData.partner_companie_active"
                :rules="rule"
                row
              >
                <template>
  ...
  methods: {
    ...
    async onAction(id) {
      if (this.$refs.form.validate()) {
        ...
        if (this.action == "add") {
          const response = await apiPartner_company.store(this.formData);
          ...
          const response = await apiPartner_company.update(
            this.id,
            this.formData
          );
          this.$refs.form.reset();
      ...
      if (this.$props.dialog) {
        if (this.action == "add") {
          this.$nextTick(() => {
            ...
            this.$nextTick(async () => {
              self.formData = await {
                partner_companie_active: "1",
              };
            });
          });
        } else if (this.action == "edit") {
          this.formDataLoading = true;
          const response = await apiPartner_company.show(this.id);
          this.$nextTick(() => {
            this.$refs.form.reset();
            const self = this;
            this.$nextTick(async () => {
