<template>
  <div>
    ...
    <v-card class="mt-5">
      <v-row class="pa-2 mt-5">
        <v-col>
          <h5>
            รวมทั้งหมด:
            {{
              Number(sumVisit).toLocaleString("th-TH", {
      ...
      <v-row>
        <v-col>
          <BarChartVisit class="mt-5" :chart-data="dataChart" />
        </v-col>
      </v-row>
    </v-card>
  </div>
...
// Bar chart
import BarChartVisit from "@/components/BarChart/BarChartVisit";
...
export default {
  ...
  async mounted() {
    await this.getDataBarVisit();
  },
  computed: {},
  methods: {
    async getDataBarVisit() {
      ...
      const responseBar = await apiDashboard.dashboardManagerBarVisit(dataBar);
      ...
      this.sumVisit = newData.reduce(reducer);
    },
