<template>
  <v-card>
    ...
    >
      ...
      <v-card-title primary-title> แจ้งยกเลิกการจอง</v-card-title>
...
export default {
  ...
  mounted() {
    this.initializeLiff(process.env.liff.cancle);
  },
  methods: {
    async checkUserLine() {
      let data = {
    ...
    async onAction(id) {
      if (this.$refs.form.validate()) {
        ...
        const response = await apiRequestCancle.store(formData);
        console.log(response.data);
        ...
        if (response.status == 200) {
          ...
          var Messages = [
            {
              type: "text",
              text: "แจ้งยกเลิกการจอง: " + response.data.car_no,
            },
            {
              type: "flex",
              altText: "แจ้งยกเลิกการจอง: " + response.data.car_no,
              contents: {
                type: "bubble",
                hero: {
                  type: "image",
                    ...
                    },
                    {
                      ...
                      action: {
                        ...
                        uri:
                          ...
                          `?type=cancle&id=${response.data.id}`,
                      },
                    },
                    {
                      type: "box",
