<template>
  <v-card>
    ...
    >
      ...
      <v-card-title primary-title>แจ้งนัดทำสัญญา</v-card-title>
...
export default {
  ...
  mounted() {
    this.initializeLiff(process.env.liff.appointment);
  },
  methods: {
    async checkUserLine() {
      let data = {
    ...
    async onAction(id) {
      if (this.$refs.form.validate()) {
        ...
        const response = await apiRequestAppointment.store(formData);
        ...
        if (response.status == 200) {
          ...
          var Messages = [
            {
              type: "text",
              text: "แจ้งนัดทำสัญญา: " + response.data.car_no,
            },
            {
              type: "flex",
              altText: "แจ้งนัดทำสัญญา : " + response.data.car_no,
              contents: {
                type: "bubble",
                hero: {
                  type: "image",
                      ...
                      contents: [
                        ...
                        },
                        {
                          ...
                          contents: [
                            ...
                            {
                              type: "text",
                              text: response.data.appointment_date,
                              wrap: true,
                              color: "#666666",
                              size: "sm",
                              flex: 5,
                    ...
                    },
                    {
                      ...
                      action: {
                        ...
                        uri:
                          ...
                          `?type=appointment&id=${response.data.id}`,
                      },
                    },
                    {
                      type: "box",
