<template>
  <v-card>
    ...
    >
      ...
      <v-card-title primary-title>แจ้งการทำสัญญา</v-card-title>
...
export default {
  ...
  mounted() {
    this.initializeLiff(process.env.liff.sign);
  },
  methods: {
    async checkUserLine() {
      let data = {
    ...
    async onAction(id) {
      if (this.$refs.form.validate()) {
        ...
        const response = await apiRequestSign.store(formData);
        ...
        if (response.status == 200) {
          ...
          var Messages = [
            {
              type: "text",
              text: "แจ้งการทำสัญญา: " + response.data.car_no,
            },
            {
              type: "flex",
              altText: "แจ้งการทำสัญญา: " + response.data.car_no,
              contents: {
                type: "bubble",
                hero: {
                  type: "image",
                  ...
                  contents: [
                    ...
                    },
                    {
                      ...
                      action: {
                        type: "uri",
                        label: "ใบงานเซนต์",
                        uri: this.serverUrl + response.data.sign_sheet,
                      },
                    },
                    {
                      type: "button",
