...
import dialogNew from "@/components/dialog/dialogSearch_term";
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "ชื่อคำ", value: "search_name", width: "15%" },
        { text: "สถานะ", value: "search_active", width: "5%" },
  ...
  methods: {
    async getData() {
      const response = await apiSearch_term.index();
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiSearch_term.destroy(id);
