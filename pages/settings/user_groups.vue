...
import dialogNew from "@/components/dialog/dialogUserGroup";
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "ตำแหน่ง", value: "user_group_name", width: "15%" },
        { text: "สิทธิ์", value: "user_group_permission", width: "15%" },
        { text: "สถานะ", value: "user_group_active", width: "5%" },
  ...
  methods: {
    async getData() {
      const response = await apiUser_groups.index();
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiUser_groups.destroy(id);
