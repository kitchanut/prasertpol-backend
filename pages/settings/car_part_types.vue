    ...
    <v-card>
      ...
      >
        <template v-slot:[`item.car_part_type_active`]="{ item }">
          <v-btn v-if="item.car_part_type_active == '1'" x-small color="success" dark>เปิดใช้งาน</v-btn>
...
import dialogNew from "@/components/dialog/dialogCarPartType";
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "ชื่อประเภท", value: "car_part_type_name", width: "15%" },
        { text: "สถานะ", value: "car_part_type_active", width: "15%" },
  ...
  methods: {
    async getData() {
      const response = await apiCar_part_types.index();
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiCar_part_types.destroy(id);
