<template>
  <div v-if="user_group_permission == -1">
    <v-card>
      ...
      >
        <template v-slot:[`item.unit_active`]="{ item }">
          <v-btn v-if="item.unit_active == '1'" x-small color="success" dark>เปิดใช้งาน</v-btn>
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "ชื่อหน่วย", value: "unit_name", width: "15%" },
        { text: "สถานะ", value: "unit_active", width: "5%" },
  ...
  methods: {
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiUnit.destroy(id);
