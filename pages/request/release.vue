  ...
  <div v-if="user_group_permission == -1">
    <v-card>
      ...
      >
        ...
        <template v-slot:[`item.sale_sheet`]="{ item }">
          <v-img width="40px" :src="serverUrl + item.sale_sheet" @click="showImg(serverUrl + item.sale_sheet)"> </v-img>
        </template>

        <template v-slot:[`item.release_img`]="{ item }">
          <v-img width="40px" :src="serverUrl + item.release_img" @click="showImg(serverUrl + item.release_img)">
          </v-img>
        </template>

        <template v-slot:[`item.insurance_font_sheet`]="{ item }">
          <v-row no-gutters>
            <v-col>
              <v-img
                width="40px"
                :src="serverUrl + item.insurance_font_sheet"
                @click="showImg(serverUrl + item.insurance_font_sheet)"
              >
              </v-img>
            </v-col>
            <v-col>
              <v-img
                ...
                @click="showImg(serverUrl + item.insurance_back_sheet)"
              >
              </v-img>
            </v-col>
          </v-row>
        ...
        <template v-slot:[`item.slip`]="{ item }">
          <v-row no-gutters>
            ...
            <v-col>
              <v-img width="40px" :src="serverUrl + item.slip" @click="showImg(serverUrl + item.slip)"> </v-img>
            </v-col>
          </v-row>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "ลำดับรถ/วันปล่อยรถ", value: "car_no" },
        // { text: "วันที่ปล่อยรถ", value: "release_date" },
        { text: "เงินที่ได้รับ", value: "dow" },
        { text: "รูปบัตร", value: "id_card" },
        { text: "เอกสาร", value: "sale_sheet" },
        { text: "รูปปล่อยรถ", value: "release_img" },
        { text: "ใบประกัน", value: "insurance_font_sheet", width: "11%" },
        { text: "ใบเสร็จ/สลิป", value: "slip", width: "11%" },
  ...
  methods: {
    ...
    async getData() {
      ...
      const response = await apiRequestRelease.indexCustom(data);
    ...
    async countData() {
      ...
      const response = await apiRequestRelease.countData(data);
    ...
    async editItem(id) {
      const respone = apiRequestRelease.update(id);
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiRequestRelease.destroy(id);
          respone.then(async (res) => {
            this.$nextTick(() => {
              this.getData();
            });
