...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      const response = await apiRequestMoneyWithdraw.indexCustom(data);
    ...
    async countData() {
      ...
      const response = await apiRequestMoneyWithdraw.countData(data);
    ...
    async editItem(id) {
      const respone = apiRequestMoneyWithdraw.update(id);
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiRequestMoneyWithdraw.destroy(id);
          respone.then(async (res) => {
            this.$nextTick(() => {
              this.getData();
            });
  ...
  watch: {
    data() {
      ...
      return this.data.map(function (item, index) {
        item.no = index + 1;
        item.bank_name = item.bank_name + "<br>" + item.account;
        item.customer_name =
          item.customer_name + "<br>" + item.customer_withdraw;
      });
    },
  },
};
