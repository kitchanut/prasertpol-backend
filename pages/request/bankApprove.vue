  ...
  <div v-if="user_group_permission == -1">
    <v-card>
      ...
      >
        ...
        <template v-slot:[`item.middle_price`]="{ item }">
          <div>{{ item.middle_price }} X {{ item.percent }}%</div>
        </template>
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "วันที่อนุมัติ", value: "approve_date", width: "9%" },
        { text: "ราคากลาง", value: "middle_price" },
        // { text: "%", value: "percent" },
        { text: "ยอดจัด", value: "finance_price" },
        { text: "ดาวน์", value: "down" },
        { text: "ราคาขาย", value: "car_price" },
  ...
  methods: {
    ...
    async getData() {
      ...
      const response = await apiRequestBankApprove.indexCustom(data);
    ...
    async countData() {
      ...
      const response = await apiRequestBankApprove.countData(data);
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiRequestBankApprove.destroy(id);
