...
export default {
  ...
  methods: {
    ...
    async getData() {
      ...
      const response = await apiRequestUpdate.indexCustom(data);
    ...
    async countData() {
      ...
      const response = await apiRequestUpdate.countData(data);
    ...
    async editItem(id) {
      const respone = apiRequestUpdate.update(id);
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiRequestUpdate.destroy(id);
          respone.then(async (res) => {
            this.$nextTick(() => {
              this.getData();
            });
