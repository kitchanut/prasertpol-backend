    ...
    <v-card>
      ...
      >
        ...
        <template v-slot:[`item.hear_from_type`]="{ item }">
          <div>{{ item.hear_from_type }}</div>
          <div>{{ item.customer_job }}</div>
        </template>
        ...
        <template v-slot:[`item.actions`]="{ item }">
          <v-btn
            v-if="item.request_booking_status == 'pedding'"
            color="warning"
            fab
            x-small
            dark
          ...
          <v-btn
            v-else-if="item.request_booking_status == 'approve'"
            color="success"
            fab
            x-small
            dark
          ...
          <v-btn
            v-else-if="item.request_booking_status == 'cancle'"
            color="primary"
            fab
            x-small
            dark
          ...
          <v-btn
            v-if="item.request_booking_status != 'cancle'"
            color="red"
            fab
            x-small
            dark
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "เงินจอง", value: "booking_fee" },
        { text: "อาชีพ/ทราบข่าว", value: "hear_from_type" },
        { text: "รูปบัตร", value: "id_card" },
        { text: "เอกสาร", value: "booking_sheet" },
        { text: "สลิป", value: "booking_slip" },
        { text: "ใบเสร็จ", value: "receipt" },
  ...
  methods: {
    ...
    async getData() {
      ...
      const response = await apiRequestBook.indexCustom(data);
    ...
    async countData() {
      ...
      const response = await apiRequestBook.countData(data);
    ...
    async editItem(id) {
      const respone = apiRequestBook.update(id);
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiRequestBook.destroy(id);
          // console.log(respone);
          respone.then(async (res) => {
            this.$nextTick(() => {
              this.getData();
