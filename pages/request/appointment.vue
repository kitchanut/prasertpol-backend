  ...
  <div v-if="user_group_permission == -1">
    <v-card>
      ...
      >
        ...
        <template v-slot:[`item.mtk_name`]="{ item }">
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: "วันที่นัด", value: "appointment_date", width: "9%" },
        { text: "ธนาคาร", value: "bank_name" },
        { text: "สาขา", value: "bank_branch_name" },
        { text: "MKT", value: "mtk_name" },
  ...
  methods: {
    ...
    async getData() {
      ...
      const response = await apiRequestAppointment.indexCustom(data);
    ...
    async countData() {
      ...
      const response = await apiRequestAppointment.countData(data);
    ...
    async editItem(id) {
      const respone = apiRequestAppointment.update(id);
    ...
    async deleteItem(id) {
      ...
      await isConfirmed.then((result) => {
        if (result == true) {
          const respone = apiRequestAppointment.destroy(id);
          respone.then(async (res) => {
            this.$nextTick(() => {
              this.getData();
            });
